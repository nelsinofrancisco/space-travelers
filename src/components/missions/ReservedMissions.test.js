...
const createComponent = create(
  <ReservedMissions missionName={"Thaicom"} />
);
...
const renderComponent = (missionName) => {
  render(
    <table className="table table-striped table-hover table-bordered">
      <tbody>
        <ReservedMissions key={uuidv4()} missionName={missionName} />
      </tbody>
    </table>
  );
}
...
describe('Testing ReservedMissions component', () => {
  ...
  test('its td HTML tag child has text as a child', () => {
    const component = createComponent.toJSON();
    console.log(component);
    expect(component.children[0].children).toEqual(["Thaicom"]);
  })
